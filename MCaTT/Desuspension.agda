...
module MCaTT.Desuspension where
  ...
  ↓GT Γ ∗ Γ⊢∗ = ∗
  ↓GT Γ (⇒ ∗ _ _) _ = ∗
  ↓GT Γ (⇒ A@(⇒ _ _ _) (Var x) (Var y)) (ar Γ⊢A Γ⊢x Γ⊢y) = ⇒ (↓GT Γ A Γ⊢A) (Var (↓Gt Γ A x Γ⊢x)) (Var (↓Gt Γ A y Γ⊢y))
...
-- end of Prelude partA x₁ x₂
  ...
  wk↓GT Γ B y (⇒ A@(⇒ _ _ _) (Var x) (Var x')) (ar Γ⊢A Γ⊢x Γ⊢x') (ar Γ+⊢A Γ+⊢x Γ+⊢x') = ⇒= (wk↓GT Γ B y A Γ⊢A Γ+⊢A) (Var= (wk↓Gt Γ B y x A Γ⊢x Γ+⊢x)) (Var= (wk↓Gt Γ B y x' A Γ⊢x' Γ+⊢x'))
  ...
  ↓wkt Γ (⇒ C x x₁) y {A = A} {t = t} Γ⊢ Γ+⊢@(cc Γ⊢' _ _) ↓Γ⊢t idp idp = wkt (transport {B = λ p → ↓GC Γ p ⊢t t # A} (has-all-paths-⊢C Γ⊢ Γ⊢') ↓Γ⊢t) (↓⊢C _ Γ+⊢)
