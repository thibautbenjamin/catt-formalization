...
-- end of Prelude partA x₁ x₂
  ...
  count-points≤ℓ : ∀ (Γ : Pre-Ctx) → count-points Γ ≤ ℓ Γ
  count-points≤ℓ ∅ = 0≤ 0
  count-points≤ℓ (Γ ∙ _ # ∗) = S≤ (count-points≤ℓ Γ)
  count-points≤ℓ (Γ ∙ _ # _ ⇒[ _ ] _) = n≤m→n≤Sm (count-points≤ℓ Γ)

  ↓ℓ : ∀ (Γ : Pre-Ctx) (Γ⊢ : Γ ⊢C) → (ℓ Γ - count-points Γ) == ℓ (↓GC Γ Γ⊢)
  ↓ℓ ∅ _ = idp
  ↓ℓ (Γ ∙ _ # ∗) (cc Γ⊢ _ _) = ↓ℓ Γ Γ⊢
  ↓ℓ (Γ ∙ x # _ ⇒[ _ ] _) (cc Γ⊢ _ _) = Sn-m (ℓ Γ) (count-points Γ) (count-points≤ℓ Γ) >> ap S (↓ℓ Γ Γ⊢)
  ...
  wk↓Gt (Γ ∙ .(ℓ Γ) # C) B .(ℓ (Γ ∙ (ℓ Γ) # C)) .(ℓ Γ) .C (var (cc Γ⊢ _ idp) (inl x∈Γ)) (var (cc _ _ idp) (inl (inr (idp , idp)))) = ⊥-elim (lΓ∉Γ Γ⊢ x∈Γ)
  wk↓Gt (Γ ∙ z # C) B .(ℓ (Γ ∙ z # C)) .(ℓ (Γ ∙ z # C)) .B (var (cc Γ⊢ _ _) (inl x∈Γ)) (var (cc _ _ idp) (inr (idp , idp))) = ⊥-elim (n∉Γ Γ⊢ (n≤Sn _) x∈Γ)
  wk↓Gt (Γ ∙ .(ℓ Γ) # C) B .(ℓ (Γ ∙ (ℓ Γ) # C)) .(ℓ Γ) .C (var (cc Γ⊢ _ idp) (inr (idp , idp))) (var (cc _ _ idp) (inl (inl lΓ∈Γ))) = ⊥-elim (lΓ∉Γ Γ⊢ lΓ∈Γ)
  wk↓Gt (Γ ∙ .(ℓ Γ) # C) B .(ℓ (Γ ∙ (ℓ Γ) # C)) .(ℓ Γ) .C (var (cc Γ⊢ _ idp) (inr (idp , idp))) (var (cc _ _ idp) (inl (inr _))) = idp
  wk↓Gt (Γ ∙ .(ℓ Γ) # C) B .(ℓ (Γ ∙ (ℓ Γ) # C)) .(ℓ Γ) .C (var (cc Γ⊢ _ idp) (inr (idp , idp))) (var (cc Γ+⊢ _ idp) (inr (contra , _))) = ⊥-elim (n≠Sn _ contra)
  ...
  ↓⊢C (Γ ∙ x # A@(_ ⇒[ _ ] _)) (cc Γ⊢ Γ⊢A idp) = cc (↓⊢C Γ Γ⊢) (↓⊢T Γ A (λ{()}) Γ⊢ Γ⊢A)  (↓ℓ Γ Γ⊢)
  ...
  ↓⊢t Γ+@(Γ ∙ .(ℓ Γ) # A@(_ ⇒[ _ ] _)) .(_ ⇒[ _ ] _) .(ℓ Γ) A≠∗ Γ+⊢@(cc Γ⊢ Γ⊢A idp) Γ+⊢A@(ar _ _ _) (var (cc _ _ _) (inr (idp , idp))) =
    var (↓⊢C Γ+ Γ+⊢) (inr (idp , (wk↓GT Γ A (ℓ Γ) A Γ⊢A Γ+⊢A ^)))
