...
module GSeTT.Uniqueness-Derivations where
  ...
  has-all-paths-∈ {Γ ∙ y # B} {x} {A} (cc Γ⊢ γ⊢A idp) (inr (idp , idp)) (inr (p' , q')) =
    ap inr (,= (is-prop-has-all-paths (is-setℕ x (ℓ Γ)) idp p') (is-prop-has-all-paths ((eqdec-is-set eqdec-PreTy) A B) idp q'))
  ...
  has-all-paths-⊢C {Γ ∙ x # A} (cc Γ⊢ Γ⊢A p) (cc Γ⊢' Γ⊢'A q) = ap³ cc ((has-all-paths-⊢C Γ⊢ Γ⊢')) ((has-all-paths-⊢T Γ⊢A Γ⊢'A)) (is-prop-has-all-paths (is-setℕ x (ℓ Γ)) p q)
  ...
  Γ⊢→length∉ : ∀ {Γ A n} → ℓ Γ ≤ n → Γ ⊢C → ¬ (n # A ∈ Γ)
  Γ⊢→length∉ {.(_ ∙ ℓ _ # _)} Sl≤n (cc Γ⊢ x₁ idp) (inl n∈Γ) = Γ⊢→length∉ (Sn≤m→n≤m Sl≤n) Γ⊢ n∈Γ
  Γ⊢→length∉ {.(_ ∙ ℓ _ # _)} Sl≤l (cc Γ⊢ x₁ idp) (inr (idp , idp)) = Sn≰n _ Sl≤l

  unique-type : ∀ {Γ t u A B} → (Γ ⊢t t # A) → (Γ ⊢t u # B) →  t == u → A == B
  unique-type {Γ ∙ _ # _} (var _ (inr (idp , idp))) (var _ (inr (idp , idp))) _ = idp
