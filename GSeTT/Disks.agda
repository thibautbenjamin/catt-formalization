...
module GSeTT.Disks where
  ...
  Pre-𝕊 O = nil
  Pre-𝕊 (S n) = (Pre-𝔻 n) :: (length (Pre-𝔻 n) , ⇒ᵤ n)
  Pre-𝔻 n = (Pre-𝕊 n) :: (length (Pre-𝕊 n) , ⇒ᵤ n)
  ...
  private
    ...
    χ_⊢ {Γ} {t ⇒[ A ] u} (ar Γ⊢A Γ⊢t:A Γ⊢u:A) =
      let Γ⊢χt = transport {B = λ n → Γ ⊢S Pre-χ A :: (n , t) > (Pre-𝕊 (dim A) :: ((length (Pre-𝕊 (dim A))) , ⇒ᵤ (dim A)))} (𝕊-length (dim A)) (sc χ Γ⊢A ⊢ (𝔻⊢ (dim A)) (trT (⇒[χ Γ⊢A ]) Γ⊢t:A) idp) in
      sc Γ⊢χt
         (𝕊⊢ (S (dim A)))
         (trT (⇒[χ Γ⊢A ] >> (wk[]T (𝕊⊢⇒ (dim A)) (transport {B = λ n → Γ ⊢S Pre-χ A :: (n-src (dim A) , t) > (Pre-𝕊 (dim A) :: (n , ⇒ᵤ (dim A)))} (𝕊-length (dim A)) Γ⊢χt) ^)) Γ⊢u:A)
         (ap S (𝕊-length(dim A)))
    ...
    ...                                     | inr _ | inl _ | inl _ =
      ...
      ::= (::=
           ...
           (×= (ap n-src (dim[] (⇒ᵤ n) _ >> (dim⇒ n)) >> ((𝕊-length _) ^)) idp))
           (×= ((S= (ap n-src (dim[] (⇒ᵤ n) _ >> (dim⇒ n)))) >> ap S ((𝕊-length _) ^)) idp)
